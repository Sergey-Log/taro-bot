...
from tarot_cards import get_random_cards, format_reading, get_single_card, get_spread_options
...
async def ask_birthdate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Запрос даты рождения с валидацией"""
    ...
    try:
        ...
        if birth_date > today:
            await update.message.reply_text(
                "❌ Дата рождения не может быть в будущем. Попробуйте ещё раз:"
            )
            return ASKING_BIRTHDATE
        
        if year < 1900:
            await update.message.reply_text(
                "❌ Год рождения должен быть после 1900. Попробуйте ещё раз:"
            )
            return ASKING_BIRTHDATE
            
    except ValueError:
        await update.message.reply_text(
            "❌ Неверная дата. Убедитесь, что дата существует (например, 31.02.1990 — недействительна). Попробуйте ещё раз:"
        )
        return ASKING_BIRTHDATE
...
async def process_spread_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    spread_id = query.data.split('_')[1]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработка основного меню расклада
    if query.data == 'do_tarot':
        await choose_spread(update, context)
        return
    
    # ... остальной код button_handler из v5.2 (полностью восстановлен) ...
    user_data = get_user_data(user_id)
    if not user_data or not user_data.get('name') or not user_data.get('birthdate'):
...
def main():
    ...
    if not TOKEN:
        ...
        return
    print("✅ Бот запущен v5.3 (выбор расклада + валидация данных)")
